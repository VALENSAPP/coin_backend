...
export class PostService {
  ...
  private async getBoostedBattleWinnerFeedItems(viewerUserId?: string, now: Date = new Date()) {
    ...
    return eligible.map((boost) => {
      ...
      return {
        ...
        end_time: null,
        raiseAmount: null,
        feedItemType: 'battle_winner_product' as const,
        battleWinnerProduct: {
          product: {
            id: product.id,
            name: product.name,
  ...
  async getAllPost(viewerUserId?: string, page: number = 1, limit: number = 20) {
    ...
    const battleWinnerItems = await this.getBoostedBattleWinnerFeedItems(viewerUserId);
